...
void float2byte(uint8_t* byteBuffer, float flt)
{
	union converter
	{
		uint8_t bytes[4];
...
// Need the map, since scaling fact     or is of float type in half precision
...
void readImage(const char* fname, value_type* imgData_h)
{
    ...
    { 
        ...
        for (int j = 0; j < width; j++)
        {
            ...
            cout << "imgData_h" << idx << ": " << imgData_h[idx] << endl;
        }
    }
...
template <class value_type>
void readImageMat(Mat &img, value_type* imgData_h)
{
	uchar* p;
	for (int i = 0; i < img.rows; ++i) // Careful it's <= to include EndRow
	{
		p = img.ptr<uchar>(i);
		for (int j = 0; j < img.cols; ++j)
		{
			int idx = IMAGE_W*i + j;
			imgData_h[idx] = (p[j]>100);
		}
	}
}
...
template <class value_type>
void saveDeviceVector(int size, value_type* vec_d, int pos)
{
    typedef typename ScaleFactorTypeMap<value_type>::Type real_type;
    value_type *vec;
    vec = new value_type[size];
...
class network_t
{
  ...
  public:
                          ...
                          int pos)
    {
        ...
        //std::cout << "Resulting weights from Softmax:" << std::endl;
        //printDeviceVector(n*c*h*w, dstData);
		saveDeviceVector(n*c*h*w,dstData,pos);
        checkCudaErrors( cudaFree(srcData) );
        checkCudaErrors( cudaFree(dstData) );
        return id;
...
int DigitRecognition()
{
    ...
    //{
        ...
        }
        {
            ...
            Layer_t<float> conv1(1,20,5,conv1_bin,conv1_bias_bin,PATH);
            ...
            while(true){
				...
				if (foundSudoku && foundLED){
					...
					for(int i=0;i<9;i++){
						...
						if((id[i]==1 && sudoku_result[i][1]<0.45) || sudoku_result[i][id[i]]<0.3 ){
							frameSwitch = true;
							...
							if (target_digit >0 && target_digit <10){
								...
								for(int i=0;i<9;i++){
									...
									if (id[i] == target_digit){
										target_point.x = sudoku_rects[i].center.x;
										target_point.y = sudoku_rects[i].center.y;
										...
										float Yaw = (float) YawAng;
										float Pit = (float) PitAng;
										uint8_t data_byte[8];
										float2byte(data_byte,Yaw);
										float2byte(data_byte,Pit);
										msg.send(data_byte,8);
										//target_sudoku = i;
										//cout << "FIRE!!!";
										finishNine = false;
										//break;
				...
				else if (!foundSudoku && foundRange && foundLED){
					//if (foundLED){
					//cout << "TEST" << endl;
						int changeCount = 0;
						for(int i=0;i<5;i++){
					...
					//}
					for(int i=4;i<=6;i++){
						currentLocations[i-4] = match_number(fire_roi, i);
					}
					int newframecount = 0;
					for(int i=4; i<=6; i++){
						if(abs(currentLocations[i-4].x - lastLocations[i-4].x) > fire_roi.cols /2.9 
						|| abs(currentLocations[i-4].y - lastLocations[i-4].y) > fire_roi.rows /2.9){
							newframecount ++;
