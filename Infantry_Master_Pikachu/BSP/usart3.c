...
/*-----USART3_TX-----PB10-----*/
/*-----USART3_RX-----PB11-----*/
Receive_Info_e pc_start_receive_flag = UNSUCCESSFUL;
uint8_t test_step=0;
...
{  
    ...
    {   
        tmp = (uint8_t)(USART_ReceiveData(USART3) & (uint8_t) 0xff);
			  if(pc_start_receive_flag ==   SUCCESSFUL)
					pc_handler(tmp);
				else if(pc_start_receive_flag == UNSUCCESSFUL)//still receive calibration data
				{
					if(FrameUnpack(tmp, rx_buffer))
					{
							...
							{
								 pc_start_receive_flag = SUCCESSFUL;
								 pc_handler(tmp);
							}
					}
					else//start getting judgement info
