...
void EncoderProcess(volatile Encoder *v, CanRxMsg * msg)
{
	...
	{
		if(v->diff < -6500)    //两次编码器的反馈值差别太大，表示圈数发生了改变 note:orginally 7500
		{
			v->round_cnt++;
			v->ecd_raw_rate = v->diff + 8192;
		}
		else if(v->diff>6500) // might incur pitch bug if too small
		{
			v->round_cnt--;
			v->ecd_raw_rate = v->diff- 8192;
		}		
	...
	v->acc = v->filter_rate - v->last_filter_rate;
	v->last_filter_rate = v->filter_rate;
	
  //v->filter_rate = 6 * (int32_t)((msg->Data[2]<<8)|msg->Data[3]); //over-write, directly read from ESC for 3510	
}
