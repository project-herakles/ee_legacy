...
*/
	CM_sref.CM1= (-ChassisSpeedRef.forward_back_ref + ChassisSpeedRef.left_right_ref);
	CM_sref.CM2 = (ChassisSpeedRef.forward_back_ref + ChassisSpeedRef.left_right_ref);
	CM_sref.CM3 = (ChassisSpeedRef.forward_back_ref - ChassisSpeedRef.left_right_ref);
	CM_sref.CM4 = (-ChassisSpeedRef.forward_back_ref - ChassisSpeedRef.left_right_ref);
...
*/
	...
	if(output_sum > 4000.0*4) //if power exceeds, reduce output proportionally
	{
		float factor = fabs(4000.0*4/output_sum);
...
{	 
	...
	if(ShootMotorPositionPID.fdb>=1)
	{
