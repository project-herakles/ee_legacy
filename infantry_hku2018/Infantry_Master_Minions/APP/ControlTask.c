...
extern Receive_Info_e judgement_process_flag;
static void power_limit_handler(void);
...
#define current_abs(__para__) (__para__)>=0?((__para__)):(__para__)
...
*/
	...
	if(speedMode != SWAY && speedMode!=DUCKING)
	{
...
*/
	...
	//power restriction from Judgement info
	power_limit_handler();
...
{
  if (judgement_process_flag == UNSUCCESSFUL)
  {
  ...
  total_cur = current_abs(CM1SpeedPID.output) + current_abs(CM2SpeedPID.output) + \
              current_abs(CM3SpeedPID.output) + current_abs(CM4SpeedPID.output);
  
  if (total_cur > total_cur_limit)
  {
    CM1SpeedPID.output = CM1SpeedPID.output / total_cur * total_cur_limit;
    CM2SpeedPID.output = CM2SpeedPID.output / total_cur * total_cur_limit;
    CM3SpeedPID.output = CM3SpeedPID.output / total_cur * total_cur_limit;
    CM4SpeedPID.output = CM4SpeedPID.output / total_cur * total_cur_limit;
  }
