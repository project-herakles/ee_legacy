...
{
	VAL_LIMIT(GimbalRef.pitch_angle_dynamic_ref, -PITCH_MAX-10.0f, PITCH_MAX+13.0f);
...
void Control_Mode_Switch(RC_Ctl_t* data)
{
  ...
  //aelse if(data->key.v&0x40)//SWITCH ACTION: q
  else if ((data->key.v&0x40)&&(!(data->key.l_v&0x40)))//SWITCH ACTION: q
	{
