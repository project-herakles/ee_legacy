...
void Remote_Control(Remote *rc)
{
	...
	{
		chassis_speed_ref.forward_back_ref = (rc->ch0 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * STICK_TO_CHASSIS_SPEED_REF_FACT;
		m_INC = (rc->ch1 - (int16_t)REMOTE_CONTROLLER_STICK_OFFSET) * 0.0005f;
	}
}
