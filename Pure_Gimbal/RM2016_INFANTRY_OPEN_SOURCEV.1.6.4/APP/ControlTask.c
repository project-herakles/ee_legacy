...
{				      
	if(GetShootState() == SHOOTING) //SHOOTING or NOSHOOTING
	{
...
float GimbalPosition_ref_supervisor = 0;
float GimbalPosition_fdb_supervisor = 0;
	...
	switch(GetWorkState())
	{
		...
		case NORMAL_STATE:
		{
			if(standbyFlag == 1)
			{
				...
				GimbalRef.yaw_angle_dynamic_ref = angleSave;
		...
		case TEST_STATE:
		{
			if(time_tick_1ms > endTime){
				if(freq>=1 && freq<22) freq+=1;
				else if(freq==22) freq=24;
				else if(freq>=24 && freq<40) freq+=2;
				else if(freq==40) freq=50;
				else if(freq>=50 && freq<120) freq+=10;
				else if(freq==120) freq=200;
				else if(freq==200) freq=250;
				else if(freq==250) freq=333;
				else if(freq==333) freq=500;
			
				endTime = time_tick_1ms + 20000/freq;//Cautious! Accuracy loss!
			}
			GMYPositionPID.ref = sineWave(60,freq,time_tick_1ms/1000.0);   //设定给定值
			GimbalPosition_ref_supervisor = GMYPositionPID.ref;
			GMYPositionPID.fdb = ZGyroModuleAngle; 					//设定反馈值
			GimbalPosition_fdb_supervisor = GMYPositionPID.fdb;
		}
	}	
}
